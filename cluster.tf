...
data "talos_client_configuration" "talosconfig" {
  ...
  endpoints            = [cidrhost(var.subnet_cidr, var.starting_ip + count.index)]
}

data "talos_machine_configuration" "machineconfig_cp" {
  count            = var.count_cp
  cluster_name     = var.cluster_name
  cluster_endpoint = "https://${cidrhost(var.subnet_cidr, var.starting_ip + count.index)}:6443"
...
resource "talos_machine_configuration_apply" "cp_config_apply" {
  ...
  node                        = cidrhost(var.subnet_cidr, var.starting_ip + count.index)
...
resource "talos_machine_configuration_apply" "worker_config_apply" {
  ...
  node                        = cidrhost(var.subnet_cidr, var.starting_ip + count.index + var.count_cp)
  depends_on                  = [proxmox_virtual_environment_vm.talos_worker]
}

resource "talos_machine_bootstrap" "bootstrap" {
  client_configuration = talos_machine_secrets.machine_secrets.client_configuration
  node                 = cidrhost(var.subnet_cidr, var.starting_ip)
...
data "talos_cluster_health" "health" {
  ...
  control_plane_nodes  = [cidrhost(var.subnet_cidr, var.starting_ip + var.count_cp - 1)]
  worker_nodes         = [for ip in range(var.starting_ip + var.count_cp, var.starting_ip + var.count_cp + var.count_worker) : cidrhost(var.subnet_cidr, ip)]
